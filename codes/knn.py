# code to classify the masked image using k-nearest neighbour ml algorithm
...
def train():
    ...
    # required folders
        ...
        pa=[1,2]#Convert to dictionary
        hyperparameters = dict(leaf_size=leaf_size, n_neighbors=n_neighbors, p=pa)
        knn_2 = KNeighborsClassifier()#Use GridSearch
        knn = GridSearchCV(knn_2, hyperparameters, cv=10)#Fit the model
        knn_ml = knn.fit(X_train, y_train)#Print The value of best Hyperparameters
        print('Best leaf_size:', knn_ml.best_estimator_.get_params()['leaf_size'])
        print('Best p:', knn_ml.best_estimator_.get_params()['pa'])
        print('Best n_neighbors:', knn_ml.best_estimator_.get_params()['n_neighbors'])
        
        print (f'Train Accuracy - : {knn_ml.score(X_train,y_train):.3f}')
        print (f'Test Accuracy - : {knn_ml.score(X_test,y_test):.3f}')
        print(knn_ml.best_params_)
        print('Model fitted...')
        ...
        df = pd.DataFrame()
        df['truth'] = y_test
        df['predict'] = knn_ml.predict(X_test)
        print('Predicting...')
        ...
        print(df.head())
        classified_data=knn_ml.predict(df)
        ...
        with open(out + r'\\knn.pkl', 'wb') as file:  # opening the file
            model= (knn_ml, y_test, y_pred)
