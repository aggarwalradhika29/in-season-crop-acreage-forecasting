# code to classify the masked image using random forest ml algorithm
...
def train():
    ...
    # required folders
        ...
        df = pd.DataFrame()
        df['truth'] = y_test
        df['predict'] = rf_Grid.predict(X_test)
        print('Predicting...')
        ...
        print(df.head())
        classified_data=rf_Grid.predict(df)
        ...
        with open(out + '\\rf.pkl', 'wb') as file:  # opening the file
            model= (rf_Grid, y_test, y_pred)
