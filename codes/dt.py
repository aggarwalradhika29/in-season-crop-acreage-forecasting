# code to classify the masked image using decision trees ml algorithm
...
def train():
    ...
    # required folders
        ...
        dt_grid = GridSearchCV(dt, param_grid=param_dict, cv=10, verbose=True)
        ...
        df = pd.DataFrame()
        df['truth'] = y_test
        df['predict'] = dt_grid.predict(X_test)
        print('Predicting...')
        ...
        print(df.head())
        classified_data=dt_grid.predict(df)
        ...
        with open(out + r'\\dt.pkl', 'wb') as file:  # opening the file
            model= (dt_grid, y_test, y_pred)
