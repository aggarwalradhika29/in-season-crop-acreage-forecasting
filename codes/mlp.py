# code to classify the masked image using multi-layer perceptron ml algorithm
...
def train():
    ...
    # required folders
        ...
        mlp_clf.fit(X_train, y_train) # X is train samples and y is the corresponding labels
        ...
        df = pd.DataFrame()
        df['truth'] = y_test
        df['predict'] = mlp_clf.predict(X_test)
        print('Predicting...')
        ...
        print(df.head())
        classified_data=mlp_clf.predict(df)
        ...
        with open(out + r'\\mlp.pkl', 'wb') as file:  # opening the file
            model= (mlp_clf, y_test, y_pred)
