# code to classify the masked image using naïve bayes ml algorithm
...
def train():
    ...
    # required folders
        ...
        print (f'Train Accuracy - : {gaussian_nb_grid.score(X_train,y_train):.3f}')
        print (f'Test Accuracy - : {gaussian_nb_grid.score(X_test,y_test):.3f}')

        print(gaussian_nb_grid.best_params_)
        print('Model fitted...')
        ...
        df = pd.DataFrame()
        df['truth'] = y_test
        df['predict'] = gaussian_nb_grid.predict(X_test)
        print('Predicting...')
        ...
        # cm.figure_.savefig(out + '\\confusionMatrix.png')
        ...
        print(df.head())
        classified_data=gaussian_nb_grid.predict(df)
        ...
        with open(out + r'\\nb.pkl', 'wb') as file:  # opening the file
            model= (gaussian_nb_grid, y_test, y_pred)
